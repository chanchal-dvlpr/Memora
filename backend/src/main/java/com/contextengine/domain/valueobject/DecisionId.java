package com.contextengine.domain.valueobject;

import com.contextengine.domain.model.Identifier;
import java.util.Objects;
import java.util.UUID;

/**
 * Represents the immutable, strongly typed unique identifier of an architectural Decision.
 * Enforces RFC-4122 Version 4 UUID rules.
 */
public record DecisionId(UUID value) implements Identifier {
    
    /**
     * Constructs a DecisionId and validates that the value is non-null and is a version 4 UUID.
     *
     * @param value the UUID value
     * @throws NullPointerException if value is null
     * @throws IllegalArgumentException if value is not a version 4 UUID
     */
    public DecisionId {
        Objects.requireNonNull(value, "DecisionId value must not be null");
        if (value.version() != 4) {
            throw new IllegalArgumentException("DecisionId UUID must be version 4");
        }
    }

    /**
     * Generates a new cryptographically secure DecisionId.
     *
     * @return a new DecisionId instance
     */
    public static DecisionId generate() {
        return new DecisionId(UUID.randomUUID());
    }

    /**
     * Parses a string representation of a UUID into a DecisionId.
     *
     * @param value the string representation of the UUID
     * @return a DecisionId instance
     * @throws NullPointerException if value is null
     * @throws IllegalArgumentException if value does not conform to string UUID representation
     */
    public static DecisionId fromString(String value) {
        Objects.requireNonNull(value, "String value must not be null");
        return new DecisionId(UUID.fromString(value));
    }
}
