...
/**
 * Strategy to detect moved files among unmatched previous and current entities.
 */
public class MoveDetectionStrategy implements SnapshotComparisonAlgorithm {
    ...
    public void compare(
        ...
        if (!config.detectMoves() || !config.compareHashes()) {
            return;
        }
                ...
                if (Objects.equals(p.fileContentHash().value(), c.fileContentHash().value())) {
                    ...
                    if (Objects.equals(pName, cName)) {
                        differences.add(new SnapshotDifference(
                            SnapshotDifferenceType.MOVED, p, c,
                            "Entity moved from " + p.filePath().value() + " to " + c.filePath().value()
                        ));
                        matchedPrev.add(p);
                        matchedCurr.add(c);
                        break;
