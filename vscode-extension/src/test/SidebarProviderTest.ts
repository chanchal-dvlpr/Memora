...
import { DashboardProvider, ArchitectureProvider, WorkProvider, MemoraTreeItem } from '../sidebarProvider';

export async function runSidebarProviderTests() {
    console.log("-> Running SidebarProviderTest...");

    // Test 1: TreeItem instantiation with parameters
    {
        const item = new MemoraTreeItem("Label1", 1, "projectContext", "Description1");
    ...
    {
        const item = new MemoraTreeItem("Label2", 0);
        assert.strictEqual(item.collapsibleState, 0);
    }

    // Test 3: DashboardProvider getTreeItem returns exact node
    {
        const provider = new DashboardProvider({} as any);
        const item = new MemoraTreeItem("Label3", 1);
    ...
    {
        const provider = new DashboardProvider({} as any);
        const projectItem = new MemoraTreeItem("Proj1", 1, "project", "p1");
    ...
    // Test 10: ArchitectureProvider getTreeItem returns element
    {
        ...
        const item = new MemoraTreeItem("ArchNode", 0);
    ...
    {
        const provider = new ArchitectureProvider();
        const parent = new MemoraTreeItem("📝 Active Design Decisions (ADRs)", 1);
    ...
    {
        const provider = new ArchitectureProvider();
        const parent = new MemoraTreeItem("⚠️ System Constraints", 1);
        const children = await provider.getChildren(parent);
        assert.strictEqual(children.length, 2);
    ...
    {
        const provider = new ArchitectureProvider();
        const parent = new MemoraTreeItem("🧠 Architectural Assumptions", 1);
        const children = await provider.getChildren(parent);
        assert.strictEqual(children.length, 1);
    ...
    // Test 15: WorkProvider getTreeItem returns element
    {
        ...
        const item = new MemoraTreeItem("WorkNode", 0);
    ...
    {
        const provider = new WorkProvider();
        const parent = new MemoraTreeItem("🚀 Features", 1);
        const children = await provider.getChildren(parent);
        assert.strictEqual(children.length, 1);
    ...
    {
        const provider = new WorkProvider();
        const parentTask = new MemoraTreeItem("📋 Tasks", 1);
        ...
        const parentBug = new MemoraTreeItem("🐛 Bugs", 1);
