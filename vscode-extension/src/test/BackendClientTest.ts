...
export async function runBackendClientTests() {
    ...
    // Test 1: constructor initialization & default configurations
    {
        ...
        assert.strictEqual((client as any).port, 9876, "Default port should be 9876");
        assert.strictEqual((client as any).token, "prod-token", "Default token should be prod-token");
    }
    ...
    // Test 2: custom configuration loading
    {
        ...
        (global as any).vscode.workspace.getConfiguration = () => ({
            get: (key: string) => {
                if (key === 'connection.port') { return 1234; }
                if (key === 'connection.token') { return 'custom-token'; }
                return undefined;
            }
        });

        const client = new BackendClient();
        assert.strictEqual((client as any).port, 1234, "Custom port should load correctly");
        assert.strictEqual((client as any).token, "custom-token", "Custom token should load correctly");
    ...
    // Test 3: update configuration dynamically
    {
        ...
        (global as any).vscode.workspace.getConfiguration = () => ({
            get: (key: string) => {
                if (key === 'connection.port') { return 5678; }
                if (key === 'connection.token') { return 'updated-token'; }
                return undefined;
            }
        });

        client.updateConfig();
        assert.strictEqual((client as any).port, 5678, "Port should update dynamically");
        assert.strictEqual((client as any).token, "updated-token", "Token should update dynamically");
    ...
    // Test 4: GET list request (getProjects)
    {
        mockHttpRequest((options, req) => {
            ...
            assert.strictEqual((options.headers as any)?.['X-Session-Token'], 'prod-token');
    ...
    // Test 24: Request header length generation checks
    {
        mockHttpRequest((options, req) => {
            ...
            assert.ok(Number((options.headers as any)?.['Content-Length']) >= 0);
            const res = new MockIncomingMessage(200, "{}");
            req.emit('response-mock', res);
        });
